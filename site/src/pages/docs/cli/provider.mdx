...
`showDeposit` belongs beside `authorizeAccessKey`, not inside it. The browser
approval carries the same funding request used by web integrations, while the
authorized key remains available to the CLI after the browser closes.
