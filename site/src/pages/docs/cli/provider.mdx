...
New account registration shows a post-connect funding prompt by default before
the user returns to the terminal. Set `capabilities.showDeposit` to `false` to
suppress it, or to `true`/object form to request the prompt explicitly.
