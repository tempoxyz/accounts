...
`privateKey` is an SDK convenience for locally managed access keys. The SDK derives public authorization parameters, stores the private key through the provider's configured storage after authorization, and does not forward `privateKey` material to wallets over JSON-RPC.
