...
When the sender has a pending locally managed access key, `Provider` attaches the pending `keyAuthorization` before fill so the returned gas covers first-use authorization. Fill and signing-only flows keep that pending authorization available for retry; sync sends clear it after the transaction is accepted, while async sends check the key state before the next access-key transaction.
