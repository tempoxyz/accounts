...
type AuthorizeAccessKeyCapability = {
  ...
  /**
   * Private key material to keep in SDK storage. The SDK derives `publicKey`
   * before forwarding the request to a wallet approval surface.
   */
  privateKey?: Hex
  publicKey?: Hex
  scopes?: {
...
`privateKey` is accepted by SDK adapters for locally managed access keys. Wallet approval surfaces should receive the derived public material, not the private key itself.
