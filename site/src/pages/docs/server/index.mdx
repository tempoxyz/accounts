---
title: Tempo Accounts Server Handlers
description: Configure server-side Tempo Accounts SDK handlers for relaying wallet RPC requests, composing backends, and managing WebAuthn ceremonies.
---

import { Cards, Card } from 'vocs'

# Handlers

Server handlers are framework-agnostic handlers that run on your backend to manage protocol operations that require server-side logic. 

Handlers are compatible with any server framework that supports the:
- [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API), and is exposed via the `Handler#fetch` function
- [Node.js `RequestListener` API](https://nodejs.org/api/http.html#http_class_http_serverrequestlistener), and is exposed via the `Handler#listener` function

```ts
import { Handler } from 'accounts/server'
import { privateKeyToAccount } from 'viem/accounts'

const handler = Handler.relay({
  feePayer: { account: privateKeyToAccount('0x...') },
})

createServer(handler.listener) // Node.js
Bun.serve(handler) // Bun
Deno.serve(handler) // Deno
app.all('*', c => handler.fetch(c.request)) // Elysia
app.use(handler.listener) // Express
app.use(c => handler.fetch(c.req.raw)) // Hono
export const GET = handler.fetch // Next.js
export const POST = handler.fetch // Next.js
```

## Choose a handler

| Handler | Use it for | Production responsibility |
| --- | --- | --- |
| [`Handler.relay`](/docs/server/handler.relay) | Fee sponsorship, transaction enrichment, automatic fee token selection, and optional auto-swap | Own the relay host, fee payer key, sponsorship policy, logs, and abuse controls |
| [`Handler.webAuthn`](/docs/server/handler.webAuthn) | Server-backed passkey registration and authentication | Own the relying party origin, credential storage, recovery policy, and challenge expiry |
| [`Handler.compose`](/docs/server/handler.compose) | Combine server handlers under one endpoint | Keep path routing explicit and test each composed handler |
| [`Handler.feePayer`](/docs/server/handler.feePayer) | Deprecated fee payer handler | Migrate to `Handler.relay({ feePayer })` |

<Cards>
  <Card
    description="Compose multiple handlers into a single handler"
    icon="lucide:layers"
    to="/docs/server/handler.compose"
    title="Handler.compose"
  />
  <Card
    description="Proxy RPC requests with sponsorship, auto-swap, and balance diffs"
    icon="lucide:radio"
    to="/docs/server/handler.relay"
    title="Handler.relay"
  />
  <Card
    description="Deprecated fee payer handler; use Handler.relay with feePayer"
    icon="lucide:circle-alert"
    to="/docs/server/handler.feePayer"
    title="Handler.feePayer"
  />
  <Card
    description="Handle WebAuthn credential registration and authentication"
    icon="lucide:fingerprint"
    to="/docs/server/handler.webAuthn"
    title="Handler.webAuthn"
  />
</Cards>

## Related guides

- [Fee sponsorship](/docs/guides/fee-sponsorship)
- [Authentication](/docs/guides/authentication)
- [Production checklist](/docs/production)
