...
declare function authorizeAccessKey(): {
  ...
  /**
   * Private key material to keep in SDK storage. Dialog-style adapters derive
   * `publicKey` before forwarding the request to the wallet approval surface.
   */
...
Alternatively, pass `privateKey` to let the SDK derive `publicKey` and store the private key through [`keyMaterialStorage`](#keymaterialstorage). Dialog-style adapters do not forward `privateKey` to the wallet approval surface.
...
Optional storage for exported access-key material. When provided, access-key metadata is persisted in `storage` and private key material is persisted separately in `keyMaterialStorage`.

```ts twoslash
import { Provider, Storage } from 'accounts'

declare const secureStorage: Storage.Storage

const provider = Provider.create({
  storage: Storage.localStorage(),
  keyMaterialStorage: secureStorage, // [!code focus]
})
```
