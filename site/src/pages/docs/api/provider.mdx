...
[Keystores](/docs/api/keystore) backing locally generated access keys, one per key type. A keystore's `createKey()` provisions key material — taking precedence over the adapter's `generateAccessKey` — and persists an opaque handle that its `toAccount()` turns back into a signing account on hydration. This enables hardware-backed access keys (e.g. Secure Enclave / Android Keystore) and other custom backends; existing `privateKey`/`keyPair` records hydrate unchanged.
