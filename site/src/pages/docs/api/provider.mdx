...
[Keystores](/docs/api/keystore) backing locally generated access keys, one per key type. A keystore's `createKey()` provisions key material and persists an opaque handle that its `toAccount()` turns back into a signing account on hydration. App-level keystores override adapter-supplied defaults (e.g. the React Native and CLI adapters default to pure-JS keystores). When `authorizeAccessKey` does not request a key type, `secp256k1` is used if a keystore for it is configured, else `p256`. Access keys created before a keystore was configured keep working. This enables hardware-backed access keys (e.g. Secure Enclave / Android Keystore) and other custom backends; existing `privateKey`/`keyPair` records hydrate unchanged.

By default, keys are non-extractable WebCrypto keys that persist through structured-clone storage (`Storage.idb`, `Storage.memory`). See [`Keystore.webCryptoP256`](/docs/api/keystore.webCryptoP256) for environments with string-based storage (React Native, Node, CLI), and [`Keystore`](/docs/api/keystore) for the interface, handle semantics, and composition guidance.
