...
Access-key configuration: `authorize` is the default authorization policy (what to request and when a stored key can be reused), `keystores` is where access-key material lives.
...
Keystores backing locally generated access keys: one per key type. A keystore's `createKey()` provisions key material — taking precedence over the adapter's `generateAccessKey` — and persists an opaque handle that its `toAccount()` turns back into a signing account on hydration.

Keystores are about the device's key capabilities: [`storage`](#storage) persists provider state, keystores hold key material. This enables hardware-backed access keys (e.g. Secure Enclave / Android Keystore) and other custom key backends. Existing records backed by `privateKey` or `keyPair` material hydrate unchanged.
...
A keystore can compose multiple backends behind one slot (e.g. a hardware keystore with a software fallback for devices without a Secure Enclave). Gate provisioning fallback on a deliberate capability check — not on errors, so a transient failure never silently downgrades a hardware-capable device to a software key — and route hydration by the handle's own discriminant:
...
Apps with custom needs (own native module, HSM, remote signer) implement the two-function keystore interface. The handle's schema is owned by whichever keystore wrote it:
...
type Keystore = {
   ...
   * keystore does not recognize or transient failures (e.g. device locked):
   * the record is kept and retried on next use. Keystores composing multiple
