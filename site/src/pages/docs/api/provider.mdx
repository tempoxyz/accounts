...
[Keystores](/docs/api/keystore) backing provider-generated access keys, one per key type. Access keys default to p256 (non-extractable WebCrypto); secp256k1 is used only when requested explicitly via `keyType`. App-level keystores override adapter-supplied defaults — the React Native adapter defaults to pure-JS p256 (no WebCrypto required) and the CLI adapter to extractable WebCrypto p256 (persists through filesystem storage). Access keys created before a keystore was configured keep working. This enables hardware-backed access keys (e.g. Secure Enclave / Android Keystore) and other custom backends; existing `privateKey`/`keyPair` records hydrate unchanged.
