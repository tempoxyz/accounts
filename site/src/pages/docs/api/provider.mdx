...
Alternatively, pass `privateKey` to let the SDK derive `publicKey` and persist the private key through the configured [`storage`](#storage). Dialog-style adapters do not forward `privateKey` to the wallet approval surface.
