...
type Entry = {
  /**
   * What the handle is made of. 'json' (default) survives any storage
   * adapter; 'structured-clone' holds live objects (e.g. a CryptoKey) and
   * persists only through structured-clone storage — on other storage the
   * handle is stripped at persist time, making the key session-only.
   */
  handle?: 'json' | 'structured-clone'
  /**
