...
- **Type:** `{ authorize?: Provider.create.AuthorizeAccessKey; keystore?: Keystore.Keystore }`
- **Optional**

Access-key configuration: `authorize` is the default authorization policy (what to request and when a stored key can be reused), `keystore` is where access-key material lives.
...
:::note
Supersedes the deprecated top-level `authorizeAccessKey` option, which remains as an alias.
:::
...
Keystore backing locally generated access keys: one entry per key type. An entry's `createKey()` provisions key material — taking precedence over the adapter's `generateAccessKey` — and persists an opaque handle that the entry's `toAccount()` turns back into a signing account on hydration.

The keystore is about the device's key capabilities: [`storage`](#storage) persists provider state, `keystore` holds key material. This enables hardware-backed access keys (e.g. Secure Enclave / Android Keystore) and other custom key backends. Existing records backed by `privateKey` or `keyPair` material hydrate unchanged.

By default, keys are generated with WebCrypto, non-extractable from creation: the private key never has a JS-visible encoding, and persists only through structured-clone storage (`Storage.idb`, `Storage.memory`) — on string-based storage the key is session-only. For environments without structured-clone storage where keys must survive restarts (React Native, Node, CLI), pass `extractable: true`: the private key is exported once as a JWK (at-rest protection is the host app's storage adapter, e.g. `secureMmkv` on React Native) and re-imported non-extractable each session. In browsers with IndexedDB storage, prefer the default.
...
const provider = Provider.create({
  accessKey: { // [!code focus]
    keystore: { p256: Keystore.webCryptoP256({ extractable: true }) }, // [!code focus]
  }, // [!code focus]
})
```

An entry can compose multiple backends behind one slot (e.g. a hardware entry with a software fallback for devices without a Secure Enclave). Gate provisioning fallback on a deliberate capability check — not on errors, so a transient failure never silently downgrades a hardware-capable device to a software key — and route hydration by the handle's own discriminant:

```ts twoslash
import { Keystore, Provider } from 'accounts'

declare function secureEnclave(options: { fallback: Keystore.Entry }): Keystore.Entry

const provider = Provider.create({
  accessKey: { // [!code focus]
    keystore: { // [!code focus]
      p256: secureEnclave({ fallback: Keystore.webCryptoP256({ extractable: true }) }), // [!code focus]
    }, // [!code focus]
  }, // [!code focus]
})
```

Apps with custom needs (own native module, HSM, remote signer) implement the two-function entry interface. The handle's schema is owned by whichever entry wrote it:
...
type Entry = {
   ...
   *
   * Throw `Keystore.KeyUnavailableError` when the key behind the handle is
   * permanently gone (e.g. hardware key deleted) — the SDK evicts the
   * record so callers re-authorize. Throw any other error for handles the
   * entry does not recognize or transient failures (e.g. device locked):
   * the record is kept and retried on next use. Entries composing multiple
   * backends should treat `KeyUnavailableError` as an ownership claim and
   * not route the handle to another backend.
   */
  toAccount(
