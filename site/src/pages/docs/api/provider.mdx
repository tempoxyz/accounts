...
Entries compose with `Keystore.fallback` — the first entry that can provision wins, and hydration routes to whichever entry recognizes the handle:

```ts twoslash
import { Keystore, Provider } from 'accounts'

declare function secureEnclave(): Keystore.Entry

const provider = Provider.create({
  keystore: { // [!code focus]
    p256: Keystore.fallback(secureEnclave(), Keystore.webCryptoP256({ extractable: true })), // [!code focus]
  }, // [!code focus]
})
```
...
type Entry = {
   ...
   * entry does not recognize (so `Keystore.fallback` can route past it) or
   * transient failures (e.g. device locked): the record is kept and
   * retried on next use.
   */
  toAccount(
