...
Keystore backing locally generated access keys. When set, access-key material is created via `keystore.createKey()` — taking precedence over the adapter's `generateAccessKey` — and persisted as an opaque, JSON-serializable handle that `keystore.toAccount()` turns back into a signing account on hydration.
...
const provider = Provider.create({
  keystore: webCryptoP256(), // [!code focus]
})
```

Apps with custom needs (own native module, HSM, remote signer) can implement the two-function interface themselves. The handle's schema is owned by whichever keystore wrote it:

```ts
type Keystore = {
  /**
   * Creates access-key material. `handle` is opaque, JSON-serializable,
   * and persisted verbatim by the SDK. Must fail loudly when runtime
   * prerequisites are missing (e.g. no Secure Enclave, no `crypto.subtle`)
   * so provisioning errors surface at authorization time, not at first sign.
   */
  createKey(options: { keyType?: 'p256' | 'secp256k1' }): Promise<{
   ...
   *
   * Throw `KeyUnavailableError` (from `accounts/keystore`) when the key
   * behind the handle is permanently gone (e.g. hardware key deleted) —
   * the SDK evicts the record so callers re-authorize. Any other error is
