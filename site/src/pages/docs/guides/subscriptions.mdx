...
The demo uses a daily plan and checks the collection route every 10 seconds. Early collection returns `null`, which shows that scheduled collection is safe to run before the next period is due.
...
Tempo subscription periods use `day` or `week` units. Run collection more often than the period so overdue subscriptions are billed promptly.
...
Call `tempo.renewSubscription` from a cron worker. It returns a receipt when it collects the next period, or `null` when the subscription is already current, missing, inactive, or superseded.
...
### Treat Collection as Idempotent

It is safe for cron to call `renewSubscription` early or more than once. A `null` result means no payment was collected, so your scheduler can continue without treating it as an error.
...
<CheckboxCards>
  ...
  <CheckboxCard
    ...
    href="#treat-collection-as-idempotent"
  >
    `renewSubscription` returning `null` is handled as nothing due.
  </CheckboxCard>
  <CheckboxCard
  ...
  <CheckboxCard
    ...
    href="#keep-periods-reviewable"
  >
    Subscription plans use Tempo-supported `day` or `week` periods.
  </CheckboxCard>
</CheckboxCards>
