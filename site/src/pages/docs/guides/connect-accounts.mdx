...
description: Connect a Tempo account in your application using the Accounts SDK and Wagmi — adapter-agnostic sign-in, sign-out, and account state.
---
...
The Accounts SDK ships several signing adapters — [Tempo Wallet](/docs/adapters/tempo-wallet), [domain-bound Passkeys](/docs/adapters/webauthn), [Turnkey](/docs/adapters/turnkey), and more. Each adapter exposes the same Wagmi connector surface, so the integration code in this guide is the same regardless of which adapter you pick. Choose an adapter once in your Wagmi config; the rest of your app stays adapter-agnostic.
...
<Cards>
  <Card
    description="Choose between Tempo Wallet, WebAuthn, Turnkey, and custom signing infrastructure"
    to="/docs/adapters"
    icon="lucide:route"
