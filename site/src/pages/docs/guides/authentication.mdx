...
<Demo
  ...
  githubUrl="https://github.com/tempoxyz/accounts/tree/main/examples/authentication"
...
The plain `email` string is convenient for display, but anyone can put any string there. To trust the email on your server, verify the signed token that ships alongside it.

### Verify the Email Server-Side

Alongside the plain `email`, the wallet returns a signed identity token bound to your app origin and the connected account. Configure your auth handler with [`identity`](/docs/server/handler.auth#identity) and the SDK forwards that token on the same verify round-trip. The handler checks it against the wallet's keys and folds the verified email onto the session.
...
const auth = Handler.auth({
  path: '/auth',
  store: Kv.durableObject(env.NONCE_DO),
  identity: { issuer: 'https://wallet.tempo.xyz' }, // [!code focus]
})

// In an authenticated route, the verified email is on the session.
app.get('/me', async (c) => {
...
The token reuses the SIWE challenge nonce, so the single-use challenge store protects both the signature and the email against replay — no extra nonce plumbing. If you mint your own session (for example in [`onAuthenticate`](/docs/server/handler.auth#onauthenticate) with `session: false`), verify the token directly with [`Handler.verifyIdentityToken`](/docs/server/handler.auth#verifyidentitytoken).
