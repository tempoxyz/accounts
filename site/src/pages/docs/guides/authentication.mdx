...
Alongside the plain `email`, the wallet returns a signed identity token bound to your app origin and the connected account, which the SDK forwards on the same verify round-trip. Your auth handler verifies it against the wallet's keys and folds the verified email onto the session with no extra configuration — use [`identity`](/docs/server/handler.auth#identity) only to point at a self-hosted wallet or to require a verified email.
...
app.get('/me', async (c) => { // [!code focus]
  ...
  return c.json({ address: session.address, email: session.email })
})
```
