...
React Native apps use the `accounts/mobileWebAuth` adapters. The provider opens Tempo Wallet in a system browser session, then receives an app-link callback with the encrypted response.
...
`accounts/mobileWebAuth/expoWebBrowser` uses Expo WebBrowser for the native
system auth session. It is an optional subpath so apps that use another browser
primitive do not install Expo WebBrowser.

## Consumer Discovery

The mobile web auth `tempoWallet()` adapter uses `https://wallet.tempo.xyz` for both Wata roles by default:
