 ...
 */
export function InteractiveQr({ value, size = 220 }: { value: string; size?: number }) {
  const [matrix, setMatrix] = useState<readonly (readonly boolean[])[] | null>(null)
...
function MatrixBridge({ onMatrix }: { onMatrix: (m: readonly (readonly boolean[])[]) => void }) {
...
function DotCanvas({ matrix, size }: { matrix: readonly (readonly boolean[])[]; size: number }) {
  ...
  useEffect(() => {
    ...
    const drawFinder = (
      ...
      ctx.beginPath()
      ctx.roundRect(originX + innerOffset, originY + innerOffset, innerSize, innerSize, innerRadius)
      ctx.fill()
    ...
    const draw = (now: number) => {
      ...
      if (!reduced) {
        if (s.active) {
          ...
          if (!last || (s.smoothX - last.x) ** 2 + (s.smoothY - last.y) ** 2 > 16) {
      ...
      for (let y = 0; y < edge; y++) {
        ...
        for (let x = 0; x < edge; x++) {
          ...
          const breath = reduced
            ? 1
            : 1 + BREATH_AMPLITUDE * Math.sin(breathPhase + hash(x, y) * 6.28)
          ...
          if (s.trail.length > 0 && !reduced) {
            ...
            if (glow > 0) {
              ...
              radius = radius + dotRadius * cap * (HOVER_SCALE - 1) * 0.5
            }
          }
