...
const INFO: Record<MethodId, { title: string; description: string }> = {
  'apple-pay': {
    title: 'Apple Pay',
    description: 'Tap-to-pay deposits via Stripe. Funds settle into the account within seconds.',
  },
  card: {
    title: 'Cards',
    description: 'Visa, Mastercard, and AmEx via Stripe. 3DS challenges handled inside the wallet.',
  },
  sepa: {
    title: 'SEPA',
    description: 'EU bank transfers with IBAN. Funds typically arrive in one business day.',
  },
  crypto: {
    title: 'Crypto',
    description:
...
export default function LocalPayments() {
  ...
  const amount = DEPOSIT_AMOUNTS.find((a) => a.id === amountId) ?? DEPOSIT_AMOUNTS[0]
      ...
      left={
        <>
          <div className="flex flex-col gap-6">
            <TogglePills options={METHODS} value={method} onChange={setMethod} label="Method" />
