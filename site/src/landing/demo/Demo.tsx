...
export default function Demo() {
    ...
    const hydrate = async () => {
      try {
        ...
        if (addr) {
          ...
          if (currentDemo === 'Spend Permissions' && statusRef.current !== 'running') {
            const permissionResult = activeSpendPermissionResult(provider, addr)
            if (permissionResult) {
  ...
  const onAction = async (variant?: string) => {
    ...
    const runPromise = def.run(provider, variant === undefined ? ctx : { ...ctx, variant })
    ...
    const pollPromise = shouldPollAccounts
      ? new Promise<{ __polled: `0x${string}` } | { __pollTimeout: true }>((resolve) => {
