...
import { PrimaryButton, SecondaryButton, useBodyAnimation } from './shared'
...
export function SpendPermissionsBody(props: DemoBodyProps) {
  ...
  const canSend = activePermission && status === 'done' && result?.complete === false && used < cap
  const done = status === 'done' && result?.complete !== false
  const nextPayment = Math.min(used + 1, cap)
  const expiresAt = result?.permissionExpiresAt ? new Date(result.permissionExpiresAt * 1000) : null
    ...
    >
      <div className="flex items-end justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-[14px] text-foreground-muted">Micro-payments</p>
      ...
      <p className="text-[14px] text-foreground-muted">
        Set a spending limit once. Matching payments can run in the background until the limit or
        expiry is reached.
      </p>
      ...
      {activePermission ? (
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 bg-panel-3 px-3 py-2">
          ...
          <p className="text-right text-[12px] text-foreground-muted">Remaining</p>
          ...
          <p className="col-span-2 font-mono text-[12px] text-foreground-muted">
            {result?.permissionSpent ? `${result.permissionSpent} used` : '$0.00 used'}
            {' · '}
            {expiresAt
      ...
      {activePermission ? (
        <SecondaryButton
          label={status === 'running' && lastVariant === 'revoke' ? 'Removing...' : 'Revoke rule'}
          status={status === 'running' ? 'running' : 'idle'}
