...
const FEE_SPONSORSHIP_RECIPIENT = '0x0000000000000000000000000000000000000001' as const
...
function receiptFeeUnits(receipt: TransactionReceipt) {
  ...
  return (effectiveGasPrice * gasUsed) / 10n ** BigInt(18 - PATH_USD_DECIMALS)
}
...
function spendPermissionBudget(
  ...
  const remaining = spentUnits >= limitUnits ? 0n : limitUnits - spentUnits
  return {
...
function spendPermissionResult(
  options: {
...
export async function connectSpendPermission(provider: AccountsProvider) {
  ...
  const permission = account ? findSpendPermission(provider, account.address) : undefined
  return {
...
export function activeSpendPermissionResult(provider: AccountsProvider, account: `0x${string}`) {
...
function findSpendPermission(provider: AccountsProvider, account: `0x${string}`) {
  ...
  return state.accessKeys?.find((key) => {
    ...
    return key.scopes?.some((scope) => {
      ...
      return scope.recipients.some(
        (recipient) => recipient.toLowerCase() === SPEND_PERMISSION_RECIPIENT.toLowerCase(),
      )
    })
  })
}

async function sendApprovedPayment(provider: AccountsProvider, account: `0x${string}`) {
...
function readCollectResponse(body: unknown): SubscriptionCollectResponse {
  ...
  if (typeof data.subscriptionId !== 'string') throw new Error('Missing subscription id.')
  if (typeof data.renewed !== 'boolean') throw new Error('Missing renewal status.')
  const receipt =
    data.receipt === null || data.receipt === undefined ? null : Receipt.Schema.parse(data.receipt)
  if (data.renewed && !receipt?.subscriptionId) throw new Error('Missing renewal receipt.')
  return {
    receipt,
...
export const DEMOS: Record<DemoKind, DemoDef> = {
  ...
  'Add Funds': {
    ...
    prelude: ['Add money to your Tempo account', 'Choose a deposit method in the wallet'],
    Body: OnRampBody,
    async run(provider) {
  ...
  'Pay Once': {
    ...
    prelude: ['Start a transfer', 'Checking recipient details', 'Ready to send'],
    Body: PayOnceBody,
    async run(provider) {
      ...
      return {
        ...
        href: tx ? `${tempoModerato.blockExplorers.default.url}/tx/${tx}` : undefined,
  ...
  'Spend Permissions': {
    ...
    prelude: ['Approve a payment rule once', 'Let Wisselbank make matching micro-payments'],
    ...
    async run(provider, ctx) {
      ...
      if (!permission && !ctx.variant?.startsWith('spend') && ctx.variant !== 'again') {
      ...
      return {
        complete: payments >= SPEND_PERMISSION_PAYMENT_COUNT ? undefined : false,
        permissionAddress: permission?.address,
        permissionExpiresAt:
  ...
  'Fee Sponsorship': {
    ...
    prelude: ['Check fee policy', 'Wisselbank covers the network fee'],
