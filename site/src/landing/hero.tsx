  ...
  return (
    <nav className="flex items-center justify-between px-6 py-6">
      ...
      <div className="flex items-center gap-7 px-3">
        <span data-hero-nav-stagger style={navStaggerStyles[1]} className="inline-flex">
          <Link
            to="/docs"
        ...
        <span data-hero-nav-stagger style={navStaggerStyles[2]} className="inline-flex">
          <a
...
function HeroIntro({ staggerStyle }: { staggerStyle: CSSProperties | undefined }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[720px] flex-col items-center gap-9 px-6 pt-24 pb-44 sm:pt-[160px]">
      ...
      <div data-hero-stagger style={staggerStyle} className="flex w-full max-w-[560px]">
        <div
...
function AdapterTabs({
  ...
  useEffect(() => {
    ...
    if (!container) return
    const button = container.querySelector<HTMLButtonElement>(`button[data-adapter="${adapter}"]`)
    if (!button) return
    setHighlight({
...
function BalancesCard({
  ...
  const cta =
    status === 'running' ? 'Opening Tempo…' : connected && address ? shorten(address) : 'Sign in'
  ...
  return (
    <div className="flex w-full max-w-[420px] flex-col gap-5 bg-panel-2 p-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          {balances.map((b, i) => (
            <div key={b.sym} className="flex items-center justify-between py-1">
...
export default function Hero({ children }: { children?: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const navItems = root.querySelectorAll<HTMLElement>('[data-hero-nav-stagger]')
    ...
    let disposed = false
    void Promise.all(animations.map((animation) => animation.then(() => undefined))).then(() => {
      if (disposed) return
      setStaggerReady(true)
    })
  ...
  const navStaggerStyles = staggerReady ? [] : HERO_NAV_FALLS.map((fall) => heroNavStyle(fall))
