...
function drawMiner(ctx: CanvasRenderingContext2D, x: number, y: number, frame: number, jumping: boolean) {
  // hardhat
...
function drawMinerDucking(ctx: CanvasRenderingContext2D, x: number, y: number, frame: number) {
  // hardhat (flat)
  ctx.fillStyle = "#f5a623";
...
function drawSaltBlock(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
...
function drawSparkle(ctx: CanvasRenderingContext2D, x: number, y: number, t: number) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      for (const b of s.salts) {
        if (b.mined) {
          if (b.sparkle < 25) {
            drawSparkle(ctx, b.x + b.w / 2, GROUND_Y - b.h / 2, b.sparkle);
          }
        } else {
          drawSaltBlock(ctx, b.x, GROUND_Y - b.h, b.w, b.h);
        }
      }

      // miner
      if (s.ducking && !s.jumping) {
        drawMinerDucking(ctx, 30, GROUND_Y - MINER_DUCK_H - 2, s.frameCount);
      } else {
        drawMiner(ctx, 30, s.minerY, s.frameCount, s.jumping);
      }

      // birds
      for (const b of s.birds) {
        drawBird(ctx, b.x, b.y, b.frame);
      }
