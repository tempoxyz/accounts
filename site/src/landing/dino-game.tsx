...
function drawSparkle(ctx: CanvasRenderingContext2D, x: number, y: number, t: number, p: Palette) {
...
function drawBird(ctx: CanvasRenderingContext2D, x: number, y: number, frame: number, p: Palette) {
  ctx.fillStyle = p.detail
  // body
...
export function DinoGame() {
  ...
  useEffect(() => {
    ...
    const initialDisp =
      Math.floor(container.getBoundingClientRect().width) || CANVAS_W_INITIAL * SCALE
    sync(initialDisp)
    ...
    const loop = () => {
      ...
      if (!s.gameOver) {
        ...
        for (const b of s.birds) {
          if (mRight > b.x + 4 && mLeft < b.x + 18 && mBottom > b.y + 2 && mTop < b.y + 12) {
            s.gameOver = true
            setGameOver(true)
            break
          }
      ...
      if (s.gameOver) {
        ...
        ctx.fillText('tap or space to restart', W / 2, CANVAS_H / 2 + 12)
      }
