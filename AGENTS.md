...
- **`with-private-fee-payer`-style Workers + Vite** — Keep in-memory stores (for example `CliAuth.Store.memory()`) at **module scope**, not inside the Worker's `fetch` handler, or every request gets a new store and sessions vanish. `@cloudflare/vite-plugin` / Miniflare can **native-crash** on secp256k1 `signTransaction` in the worker (`fetch failed` at `_Miniflare.dispatchFetch`), which bypasses JavaScript `try/catch`; running the fee-payer/signing API on **Bun** (or another non-workerd path) avoids that. When **Vite** serves the frontend and **Bun** (or similar) serves API routes, add Vite **`server.proxy`** for those paths (or serve app and API from one origin) so routes like `/fee-payer` do not 404.
