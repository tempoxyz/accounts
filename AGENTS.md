...
- **Mirror new `catalog:` deps into the SDK workspace catalog** — if a package under `ref/sdk` adds a dependency as `"catalog:"`, add it to `ref/sdk/pnpm-workspace.yaml` too or `pnpm install --frozen-lockfile` in the SDK Docker/dev workspace will fail before the playground boots.
