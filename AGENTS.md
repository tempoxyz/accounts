...
- **React Native consumers need built entrypoints and browser guards** — Expo/Metro should receive `react-native` export conditions that point at `dist/*`, and browser-only provider announcement logic must guard both `window` and `CustomEvent` because React Native may expose `window` without browser event constructors.
- **`run_worker_first` route patterns must be exhaustive** — Cloudflare playgrounds need every API route listed, including both exact base paths like `/cli-auth` and wildcard children like `/cli-auth/*`, or POSTs can fall through to the SPA/static layer and return 405.
...
- **CLI auth example URLs are configurable and wallet-hosted paths live under `/embed/cli-auth`** — support both `--url` and `AUTH_URL` defaults, avoid hardcoded personal hostnames, and when targeting the real wallet use the `/embed/cli-auth` aliases.
- **In-memory example stores must be module-scoped** — when examples rely on `Kv.memory()` or other in-memory stores for sessions, create them once at module scope instead of inside `fetch`; per-request stores lose session state immediately.
- **Cookie-backed fee-payer relays need explicit credentials** — browser relay transports like `http(feePayer)` must pass `fetchOptions: { credentials: 'include' }` when authorization depends on an HttpOnly session cookie, or requests to `/fee-payer` will arrive unauthenticated.
