...
- **`VITE_NODE_TAG`** — accepts a Docker image tag (e.g. `latest`, `sha-abc123`), an HTTP RPC URL (e.g. `https://rpc.moderato.tempo.xyz`) that resolves to a `sha-<hash>` tag via `web3_clientVersion`, or a local Tempo binary path for native localnet testing.
