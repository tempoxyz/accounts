...
- **This repo is embedded as wallet-next's `ref/sdk` submodule** -- wallet-next links `accounts: link:./ref/sdk` and its docker dev stack installs `ref/sdk` standalone. Relative `link:` overrides in `pnpm-workspace.yaml` (for example `link:../../some-local-repo`) resolve differently inside that embedding and break the install; keep dependencies resolvable from the registry on shared branches.
