...
export function codeAuth(options: codeAuth.Options = {}): Handler {
  ...
  router.get(`${path}/pending/:code`, async (c) => {
    try {
      ...
      const result = await cli.pending({ code })
  ...
  router.post(`${path}/code`, async (c) => {
    try {
      ...
      getClient(chainId)
      const result = await cli.createDeviceCode({ request })
  ...
  router.post(`${path}/poll/:code`, async (c) => {
    try {
      ...
      const code = c.req.param('code')
      const result = await cli.poll({ code, request })
  ...
  router.post(path, async (c) => {
    try {
      ...
      const result = await cli.authorize({
