...
describe('request', () => {
  ...
  test('default: returns active account request metadata', () => {
    const store = setup([
      {
        address: accounts[0].address,
        keyType: 'webAuthn_headless',
    ...
    `)
    expect(address).toMatchInlineSnapshot(`"${accounts[0].address}"`)
  })

  test('behavior: falls back to address-only metadata when signing material is unavailable', () => {
    const store = setup([{ address: accounts[0].address }])
  ...
  test('behavior: includes matching access key metadata', () => {
    ...
    `)
    expect(accessKeyAddress?.toLowerCase()).toMatchInlineSnapshot(
  ...
  test('behavior: can force root account metadata', () => {
    ...
    const result = Account.request({ accessKey: false, store })
