...
- `playground/src/cli-auth.tsx` renders the approval UI inside the normal playground app.
...
1. pubkey-only requests with server defaults
2. real `tempo/app` adoption of `Handler.cliAuth`
3. language-agnostic protocol documentation
4. scoped access keys as a later gated extension
