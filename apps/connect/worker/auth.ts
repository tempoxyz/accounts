...
export const auth = new Hono<{ Bindings: Env }>()
  ...
  .post('/otp/send', zValidator('json', z.object({ email: z.email() })), async (c) => {
  ...
  /** `POST /otp/verify` — verify code, upsert user, mint session cookie, return user + wallet state. */
  .post(
    '/otp/verify',
    ...
    async (c) => {
      ...
      const user = await User.upsert(db, email)
      const wallets = await Wallet.getByUserId(db, user.id)
      const wallet = wallets.findLast((w) => w.credentialId && w.publicKeyHex)

      // When returning user has a wallet, require passkey auth before granting full session.
      if (wallet) {
        await Session.set(c, process.env.SESSION_PRIVATE_KEY!, user.id, { email, embed: true })
        return c.json({
          user: { id: user.id, email },
          credentials: wallets.filter((w) => w.credentialId).map((w) => ({ id: w.credentialId })),
        })
      }

      await Session.set(c, process.env.SESSION_PRIVATE_KEY!, user.id, { email, embed: true })
      return c.json({ user: { id: user.id, email } })
    },
  )
